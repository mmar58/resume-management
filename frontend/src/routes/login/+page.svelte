<script lang="ts">
  ...
  async function handleLogin(e: Event) {
    ...
    try {
      ...
      errorMsg = err.message || 'Failed to login';
    } finally {
      loading = false;
    }
  }
...
<div class="flex items-center justify-center min-h-[80vh]">
  <div class="w-full max-w-md p-8 rounded-2xl backdrop-blur-xl bg-slate-900/60 border border-slate-700/50 shadow-2xl">
    <div class="text-center mb-8">
      <h1 class="text-3xl font-black bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 to-purple-400">Welcome Back</h1>
      <p class="text-slate-400 mt-2">Sign in to your CV Nexus account</p>
    </div>
    ...
    <p class="mt-8 text-center text-sm text-slate-400">
      ...
      <a href="/register" class="font-semibold text-indigo-400 hover:text-indigo-300 transition-colors">Sign up</a>
    </p>
  </div>
</div>
